    ...
    if "file" in request.files and request.files["file"].filename:
        ...
        if ext in (".pdf", ".md"):
            ...
            try:
                ...
                # nothing. We don't auto-OCR; guide the user to paste the text
                # or upload a text-based file instead.
                if ext == ".pdf" and not (raw_text or "").strip():
                    flash("This PDF looks scanned (no selectable text), so it can't be "
                          "read automatically. Paste the bill text below, or upload a "
                          "text-based PDF or .txt file instead.", "error")
                    return redirect(url_for("index"))
            except Exception:
